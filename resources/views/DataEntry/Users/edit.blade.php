    ...
    <div class="container-fluid">
        <div class="row bg-title">
            ...
            <div class="col-lg-9 col-sm-8 col-md-8 col-xs-12">
                <ol class="breadcrumb">
                    <li><a href="/dashnoard">لوحة التحكم</a></li>
                    <li><a href="/dashboard/users">كل الأعضاء</a></li>
                    <li class="active">تعديل {{$user->name}}</li>
                </ol>
            </div>
            <!-- /.col-lg-12 -->
        </div>
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="white-box">
                    <h3 class="box-title m-b-0">تعديل {{$user->name}}</h3>
                    <div class="row">
                        <div class="col-sm-12 col-xs-12">

                            {!!Form::model($user , ['route' => ['editors.update'] , 'method' => 'PATCH','files'=>true]) !!}
                            @include('DataEntry.Users.form')
                            {!!Form::close() !!}
                        </div>
                    </div>
                </div>
...
@section('footer')
    ...
    <script>
        jQuery(document).ready(function() {
            ...
            $('.type').val('{{$user->role}}').trigger('change');
            $('.status').val('{{$user->status}}').trigger('change');
            $('.cat').val({{$cat}}).trigger('change');
